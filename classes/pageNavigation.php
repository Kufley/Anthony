...
class mosPageNav 
{
	...
	function getPagesLinks()
	{
		...
		for ($i=$start_loop; $i <= $stop_loop; $i++){
			$page = ($i - 1) * $this->limit;
			if ($i == $this_page) {
				$html .= "\n<span><font style='font-size:12px;background-color:#652084;border-color:#000000;color:#FFFFFF;'>&nbsp;<b>$i</b>&nbsp;</font></span>";
			} else {
	...
	function getListFooter()
	{
		...
		if($this->getPagesCounter()=='No records found.'){
			$display='';
				...
				$html=' <div id="bottom-links" class="paging">'.$this->getLimitBox().'<div class="paging_l"><span>'.$display.' '.$this->getPagesCounter().'</span></div><div class="paging_r">'.$this->getPagesLinks().'</div></div>';
